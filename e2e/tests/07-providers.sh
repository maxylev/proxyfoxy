...
proxyfoxy add pm_user pm_pass 8093 residential > /dev/null 2>&1 || {
  fail "add residential proxy"
  return
}
sleep 2
wait_for_port 9000 20 || {
...
if [ -z "$TOKEN" ]; then
  fail "provider token stored for provider tests"
  proxyfoxy delete pm_user 8093 > /dev/null 2>&1 || true
  return
fi
proxyfoxy provider 127.0.0.1:9000:"$TOKEN" --quiet > /tmp/proxyfoxy-provider-07.log 2>&1 &
PROVIDER_PID=$!
pass "provider started for provider tests"
...
for i in $(seq 1 30); do
  if [ -f /var/run/proxyfoxy_state.json ]; then
    ...
    if echo "$STATE" | grep -q '"ip"'; then
      PROVIDER_IP=$(echo "$STATE" | grep -o '"ip"[[:space:]]*:[[:space:]]*"[^"]*"' | head -1 | cut -d'"' -f4)
      break
    fi
  fi
  sleep 1
...
proxyfoxy providers block "$PROVIDER_IP" suspicious > /dev/null 2>&1 || {
  fail "block provider"
  return
}
...
proxyfoxy providers unblock "$PROVIDER_IP" > /dev/null 2>&1 || {
  fail "unblock provider"
  return
}
...
proxyfoxy providers whitelist "$PROVIDER_IP" > /dev/null 2>&1 || {
  fail "whitelist provider"
  return
}
...
proxyfoxy providers unwhitelist "$PROVIDER_IP" > /dev/null 2>&1 || {
  fail "unwhitelist provider"
  return
}
